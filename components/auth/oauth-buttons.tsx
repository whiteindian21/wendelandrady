...
/**
 * Shared Google/GitHub OAuth buttons. Both providers redirect through
...
export function OAuthButtons({ next }: { next: string }) {
  ...
  async function signIn(provider: "google" | "github") {
    ...
    try {
      ...
      // On success the browser navigates to the provider — no state reset.
    } catch {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          {loading === "google" ? <Loader2 className="animate-spin" aria-hidden="true" /> : <GoogleIcon />}
          Google
        </Button>
        <Button
          type="button"
        ...
        >
          {loading === "github" ? <Loader2 className="animate-spin" aria-hidden="true" /> : <GithubIcon />}
          GitHub
        </Button>
      </div>
    </>
